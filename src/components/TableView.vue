...
<table width="100%" v-if="currentPageData">
    <tr><td class="totalPages" :colspan="labels.length" v-if="showTotalPages && this.paginationOption.showPagination">
     <p class="control is-expanded">
        ...
        <span v-if="showSearch || showFilter" class="help is-dark"><strong>{{filteredItems.length}}</strong> of {{dataSets.length}} record(s) found</span>
    </p>
...
export default {
    ...
    searchInTheList: function searchInTheList(searchText, currentPage) {
        var self = this;
        if (!searchText) {
            this.filteredItems = this.dataSets.filter(function (v, k) {
            return true;
            });
        } else {
            this.filteredItems = this.dataSets.filter(function (v, k) {
